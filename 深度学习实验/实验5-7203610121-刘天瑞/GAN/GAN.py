...
def Train():
    for ep in range(epoch):
        ...
        for i in range(int(train_size / batch_size)):
            ...
            outFake = D(fake)
            # 计算Generator的loss
            loss_G = torch.mean(torch.log(1. - outFake))
            # 更新生成器
            ...
            loss_D = - torch.mean(torch.log(outReal) + torch.log(1. - outFake))
            # 更新判别器
            optimizer_D.zero_grad()
            loss_D.backward()
            optimizer_D.step()
        print("epoch: {:d}     discriminator_loss: {:.3f}     generator_loss: {:.3f} ".format(ep + 1, loss_D, loss_G))
        if (ep + 1) % 10 == 0:
            Test_Generator()
