...
from RNNcell_series import LSTM_cell, GRU_cell, RNN_cell
...
class RNN(nn.Module):
    ...
    def Forward(self, input, hx = None):
        ...
        # Input of shape (batch_size, seqence length, input_size)  BATCH FIRST
        #
        # Output of shape (batch_size, output_size)  x
...
class LSTM(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, bias, output_size):
        ...
        self.rnn_cell_list.append(LSTM_cell(self.input_size,self.hidden_size,self.bias))
        for l in range(1, self.num_layers):
            self.rnn_cell_list.append(LSTM_cell(self.hidden_size,self.hidden_size,self.bias))
...
class GRU(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, bias, output_size):
        ...
        self.rnn_cell_list.append(GRU_cell(self.input_size,self.hidden_size,self.bias))
        for l in range(1, self.num_layers):
            self.rnn_cell_list.append(GRU_cell(self.hidden_size,self.hidden_size,self.bias))
        self.fc = nn.Linear(self.hidden_size, self.output_size)
...
class BidirRecurrentModel(nn.Module):
    def __init__(self, mode, input_size, hidden_size, num_layers, bias, output_size):
        ...
        if mode == 'LSTM':
            ...
            self.rnn_cell_list.append(RNN_cell(self.input_size,self.hidden_size,self.bias,"tanh"))
            for l in range(1, self.num_layers):
                self.rnn_cell_list.append(RNN_cell(self.hidden_size,self.hidden_size,self.bias,"tanh"))

        elif mode == 'RNN_RELU':
            self.rnn_cell_list.append(RNN_cell(self.input_size,self.hidden_size,self.bias,"relu"))
            for l in range(1, self.num_layers):
                self.rnn_cell_list.append(RNN_cell(self.hidden_size,self.hidden_size,self.bias,"relu"))
        else:
            raise ValueError("Invalid RNN mode have been selected.")
